        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
                    ...
                    <center><h1>Automation Execution Report</h1></center><center>
					<table style="border-style: none;" cellspacing="1" cellpadding="8">
						<tr style="border-style: none;">
							<td style="border-style: none;">
								<h1>
									Instance :  <xsl:value-of select="$testCaseElements[1]/class[1]/test-method[1]/params/param[@index='2']/value[1]"/> 
								</h1>
							</td>

							<td style="border-style: none;" >
								<h1>
									Date:  <xsl:value-of  select="format-dateTime(current-dateTime(), '[Y1]-[M01]-[D01]')"/> 
								</h1>
							</td>
							<td style="border-style: none;">
								<h1>
									Time:  <xsl:value-of  select="format-dateTime(current-dateTime(), '[h].[m01][Pn]')"/>  
								</h1>
							</td>
						</tr>
					</table> </center>         
                    ...
                    <table width="100%" cellpadding="5" cellspacing="1">
						<tbody>
							<tr class="TCStatusHeader" style="background-color: #eaf0f7; font-size: 16px;">
								<td width="100%"><b>Test Scenario's</b></td>
								<td nowrap="true"><b>Status</b></td>
								<td nowrap="true" align="center"><b>Report</b></td>
							</tr>
							
							<xsl:for-each select="$suiteElements">
								<xsl:variable name="testCaseElements"
                                      select="if (test/@url) then document(test/@url)/test else test"/>
								<xsl:for-each select="$testCaseElements/class[descendant::test-method[not(@is-config) and @status='FAIL']]">
									<xsl:variable name="TestCaseName" select="tokenize(@name,'\.')[last()]" />
									<xsl:variable name="TestCaseStatus" select="./test-method[not(@is-config)]/@status" />
									<xsl:variable name="ReportHTML" select="concat($TestCaseName,'.html')"/>
									<tr style="background-color: #f5f5f5; font-size: 12px;">
										<td>
											<b><xsl:value-of select="$TestCaseName"/></b>
										</td>
										<td align="center" bgcolor="#ffbbbb">									
											<xsl:value-of select="$TestCaseStatus"/>
										</td>
										<td align="center" nowrap="true">
											<a onclick="toggleDetailsVisibility('{$TestCaseName}')">Complete Report</a>
										</td>
									</tr>
									<tr>
										<td colspan="3" class="detailsBox">
											<div id="{$TestCaseName}" class="testMethodDetails">
								...
								<xsl:for-each select="$testCaseElements/class[descendant::test-method[not(@is-config) and @status='PASS']]">
									<xsl:variable name="TestCaseName" select="tokenize(@name,'\.')[last()]" />
									<xsl:variable name="TestCaseStatus" select="./test-method[not(@is-config)]/@status" />
									<xsl:variable name="ReportHTML" select="concat($TestCaseName,'.html')"/>
									<tr style="background-color: #f5f5f5; font-size: 12px;">
										...
										<td align="center"  nowrap="true">
											<!-- <a href="https://github.com/pgupta26/Gmail/blob/master/testng-results.xsl" target="_blank">Complete Report</a> -->
											<a onclick="toggleDetailsVisibility('{$TestCaseName}')">Complete Report</a>
										</td>
                                    </tr>
									<tr>
