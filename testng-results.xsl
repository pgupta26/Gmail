                    ...
                    <table width="100%" cellpadding="5" cellspacing="1">
						<tbody>
							<tr class="TCStatusHeader" style="background-color: #eaf0f7; font-size: 16px;">
								<td width="100%"><b>Test Scenario ID</b></td>
								<td nowrap="true"><b>Status</b></td>
								<td nowrap="true" align="center"><b>Comments</b></td>
							</tr>
							
							<xsl:for-each select="$suiteElements">
								<xsl:variable name="testCaseElements" select="if (test/@url) then document(test/@url)/test else test"/>
									 <xsl:for-each select="$testCaseElements">
										<xsl:variable name="classElements" select="if (class/@url) then document(class/@url)/class else class"/>
											<xsl:for-each select="$classElements">
												<tr style="background-color: #f5f5f5; font-size: 12px;">
													<td>
													...
													<td align="center" nowrap="true">
														<a href="${env.AUTOMATION_PATH}/${env.Project_Name}/testng-xslt/SaffuseTest.html" target="_blank">Complete Report</a>
													</td>
												</tr>
											</xsl:for-each>
									</xsl:for-each>        
